...
def get_log(name,path,ct,date,shape,buy_color,sell_color,opac):
    ...
    for i, row in test.iterrows():
        if i == 0:
            ...
            continue
        if row['cn_lot'] != 0 and test.loc[i-1,'cum_lot']%19 == 0:
            order_id = order_id + 1
        test.loc[i,'order_id'] = order_id
...
#*************************************     Calc Theo Data    ******************************************#
    ...
    agdata['Theo_A50'] =  agdata['XIN9I Index'] * (agdata['FFB1 Index'] ) / (agdata['SSE50 Index'])
    agdata['Base'] = base
...
#*************************************     Read  Log    ******************************************#
    if_pathmap = {}
    if_pathmap['tj'] = 'E:\\tj\\tj_股指\\Algo_kk_ih\\'
    if_pathmap['sy'] = 'E:\\sy\\sy_股指\\Algo_kk_ih\\'
    if_pathmap['ty'] = 'T:\\index_quoter_rebalanced\\Algo_kk_ih\\'
    if_pathmap['xx'] = 'X:\\Algo_kk_ih\\'
    if_pathmap['dy'] = 'Y:\\Algo_kk_ih\\'
    if_pathmap['gxm'] = 'Z:\\Algo_kk_ih\\'
...
#*************************************     Draw  Chart    ******************************************#
    ...
    a50_line = (
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(title="IH Trade Log -- "+d),
...
def ih_trading_chart(d,b):
    ...
    for d in dates:
        if os.path.exists('.\\股指数据\\'+d+'.xlsx'):
            ...
            tl.add(g,d)
    tl.render('.\\templates\\IH_log.html') 
    return tl

if __name__ == '__main__':
    ih_trading_chart('2021-01-20',-40)
